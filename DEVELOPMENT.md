...
- **`zip-utils.js`** : Utilitaires pour l'extraction sécurisée de fichiers ZIP, incluant une protection stricte contre les attaques par Path Traversal et vérifiant la signature magique (`0x504B0304`).
...
1. **Authentification Hardware-Bound (`Auth.js`)** : 
   ...
   - Sel : Fichier `salt.key` aléatoire généré au premier lancement.
   - Mot de passe : Nom d'hôte matériel (`os.hostname()`) + `_GensUser`.
   *Le vol du dossier Horizon sans la machine physique rend le déchiffrement virtuellement impossible.*
...
2. **Intégrité de Synchronisation (`lock.js`)** : 
   ...
   - Mécanisme : Le fichier `horizon.lock` contient le PID du processus maître.
   - "Stale Lock" : Un lock est considéré comme périmé s'il date de plus de 2 heures (`STALE_LOCK_MS = 7200000`) et que le processus PID n'est plus en cours d'exécution. Il est alors purgé automatiquement.
